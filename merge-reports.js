...
const coverage = JSON.parse(fs.readFileSync(path.resolve('coverage/coverage-final.json'), 'utf8'));

// Read the mocha test results
const mochaResults = JSON.parse(fs.readFileSync(path.resolve('mocha-results.json'), 'utf8'));
  ...
  testResults: mochaResults.tests.map(test => {
    ...
    return {
      assertionResults: [{
        ...
        failureMessages: test.err ? [test.err.message || String(test.err)] : [],
      ...
      endTime: new Date(test.duration ? new Date(mochaResults.stats.start).getTime() + test.duration : 0).getTime(),
...
fs.writeFileSync(path.resolve('report.json'), JSON.stringify(jestReport, null, 2));
