...
export class EthereumProvider {
  ...
  request(payload) {
      ...
      return new Promise((resolve, reject) => {
          setTimeout(() => {
              const result = (Array.isArray(this.returnData)) ? this.returnData.find(e => { return e.key === keyParam}).value : this.returnData
...
export class ApolloClient {
  ...
  query(props) {
      ...
      if (!this.getAll && Array.isArray(this.returnData)) {
          ...
          val = this.returnData.find(e => { return e.key === keyParam }).value
